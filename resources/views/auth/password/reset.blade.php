    ...
    <div class="2xl:w-1/4 lg:w-1/3 md:w-1/2 w-full">
        <div class="card overflow-hidden sm:rounded-md rounded-none">
            <form method="post" action="{{ route('password.update') }}" class="p-6">
                ...
                <input value="{{ request()->query('email') }}" name="email" class="form-input" type="hidden">
                <input id="email" required value="{{ request()->token }}" name="token" class="form-input" type="hidden">

                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-600 dark:text-gray-200 mb-2" for="password">New Password</label>
